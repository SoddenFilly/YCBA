...
class UI(QMainWindow):
    def __init__(self):
        ...
        uic.loadUi("resources/christmas.ui", self)
    ...
    def click_pushButton(self):
        ...
        Animate.opacity(self, 1, 0.8, 70, self.pushButton, True)
        codeInput = self.lineEdit.text().lower()
        # print(codeInput)
        
        if codeInput == "buzzinga!":

            uic.loadUi("resources/christmas2.ui", self)

            self.pushButton.clicked.connect(self.click_pushButton2)
        # STATIC()

    def click_pushButton2(self):
        global once
        if once == 1:
            once = 0
            Animate.opacity(self, 1, 0.8, 75, self.pushButton, True)
            self.pushButton.setText("2QVZI-PLDV5-252IH")
